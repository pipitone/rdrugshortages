...
dpd_load = function(data_dir='data/') {
  ...
  for (table_name in names(tables)) {
    ...
    dpd[[table_name]] =
      dir(data_dir, pattern = paste0(table_name, '.*.txt$'), full.names = T) %>%
        purrr::keep(function(x) file.size(x) > 0) %>%
        purrr::map(readr::read_csv,
                   col_names = headers,
                   locale = readr::locale('en', date_format = '%d-%b-%Y'),
                   col_types = purrr::map(headers,  function (x) # guess base on name
                      dplyr::case_when(endsWith(x, '_DATE') ~ 'D',
                                x == 'DRUG_CODE' ~ 'i',
                                T ~ 'c')) %>%
